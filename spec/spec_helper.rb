...
def loadWorkerTestConfig(local_db_file)
  config_file = File.join(config_base_dir, "wtp_worker.yml")
...
def getNodeTestConfig
  config_file = File.join(config_base_dir, "wtp_node.yml")
  ...
  options = {
    ...
    :wtp                   => parse_property(config, "wtp", Hash),
    ...
    :base_dir        => "/tmp/wtp_instances",
    :local_db_file   => "/tmp/wtp_node_" + Time.now.to_i.to_s + ".db",
    :service_log_dir => "/tmp/wtp_instances/log",
    :image_dir       => "/tmp/wtp_image",
    :max_disk        => 10,
